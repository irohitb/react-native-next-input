...
const NextTextInput = ({ noOfTextInput, placeholder, displayColum, keyboardType, textInputStyle, onChangeValue, parentViewStyle, value, clearInput, onInputCleared }) => {
    ...
    return (React.createElement(View, { style: displayColum
            ...
            : [{ display: 'flex', flexDirection: 'row' }, parentViewStyle] }, inputRefs.map((element, index) => (React.createElement(TextInput, { key: index, placeholder: `${placeholder[index]}`, value: inputValues[index], maxLength: 1, keyboardType: keyboardType, style: textInputStyle, ref: inputRefs[index], numberOfLines: 1, onChangeText: (text) => onChangeHandler(text, index) })))));
};
